...
const AppliedFilter = ({ appliedFilter = {}, filterConfig, onRemoveFilter, onClearFilter }) => {
  ...
  if (applicableFilter.length > 0) {
    const appliedFilterDisplay = applicableFilter.map(([key, value]) => {
      const filterItem = filterConfig.fields
        .filter((field) => field.isVisible || field.isVisible === undefined)
        .find((item) => {
      ...
      if (filterItem.type === 'checkbox-group') {
        ...
        displayValue = filterItem.displayValue ? value[filterItem.displayValue] : value.text;
      } else if (filterItem.type === 'toggle') {
        displayValue = value ? t(filterItem.labelB) : t(filterItem.labelA);
      } 
      else if (filterItem.type === 'date') {
        displayValue = value[0].toLocaleDateString() ;
      } else {
        displayValue = value;
      }
