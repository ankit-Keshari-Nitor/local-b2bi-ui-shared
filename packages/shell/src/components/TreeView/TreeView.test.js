...
describe('TreeView', () => {
  describe('renders as expected - Component API', () => {
    it('should spread extra props onto outermost element', () => {
      ...
      expect(screen.getByRole('tree')).toHaveAttribute('data-testid', 'test-id');
    });
    ...
    it('should support a custom `className` prop on the outermost element', () => {
      const { container } = render(<TreeView className="custom-class" label="Tree" />);
    ...
    it('should respect hideLabel prop', () => {
      ...
      expect(screen.queryByText('Tree View', { selector: 'label' })).not.toBeInTheDocument();
    });
    ...
    it('should respect multiselect prop', async () => {
      ...
      expect(lists[1]).toHaveAttribute('aria-selected', 'false');
    });
    ...
    it('should respect onSelect prop', () => {
      ...
      render(
        <TreeView label="Tree View">
          <TreeNode onSelect={onSelectSpy} id="Node 1" data-testid="Node 1" label="Node 1" />
        </TreeView>
      );
    ...
    it('should respect selected prop', () => {
      ...
      expect(screen.getByRole('treeitem', { name: 'Node 1' })).toHaveClass(`${prefix}--tree-node--selected`);
    });
    ...
    it('should respect showCheckbox prop', () => {
      render(
        <TreeView label="Tree View" showCheckbox={true}>
          <TreeNode id="Node 1" data-testid="Node 1" label="Node 1" />
        </TreeView>
      );

      expect(screen.getByRole('treeitem', { name: 'Node 1' })).toHaveClass(`${prefix}--tree-node ${prefix}--tree-leaf-node`);
    });

    it('should respect additionalInfo prop', () => {
      render(
        <TreeView label="Tree View">
          <TreeNode id="Node 1" data-testid="Node 1" label="Node 1" additionalInfo={<>{'2'}</>} />
        </TreeView>
      );
      //
      expect(screen.getByRole('treeitem')).toHaveClass(`${prefix}--tree-node--with-additional-info`);
    });
  });
  ...
  describe('behaves as expected', () => {
    ...
    it('should render tree with disabled nodes', () => {
      render(
        <TreeView label="Tree View">
          ...
          <TreeNode data-testid="Node 2" label="Node 2" disabled={true} isExpanded={true}>
    ...
    it('should render tree with icons', () => {
      ...
      expect(screen.getByTestId('Node 1')).toHaveClass(`${prefix}--tree-node--with-icon`);
    });
